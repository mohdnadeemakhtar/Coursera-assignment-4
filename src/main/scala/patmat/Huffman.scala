...
object Huffman {
   ...
   * character and an integer. Pairs can be cons  tructed easily using parentheses:
   *
   ...
   */
    ...
    chars.sortWith(_<_).groupBy(c => c).map { xs =>
      (xs._1 ,xs._2.length)
    }
  }
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = until(singleton, combine)(makeOrderedLeafList(times(chars).toList)).head
